...
#===============================================================================
"""
This is a python script for reading binary files from F3D program. This script 
is useful for reuse if you need to read through multiple time slices of a 
single file and then loop through the files. The outer loop is files and inner 
loop is time slices.
|Author: Regis | Date Created/Last Modified: Oct 6, 2023/Aug 24, 2024|
"""
#===============================================================================
#-------------------------------------------------------------------------------
# Imports
...
class Config:
    ...
    """
    # Flags:
    multi_flag_ = False # Set to True to load multiple time slices.
...
def main():
    ...
    if Config.interactive_flag:
        multiread(plane=Config.plane_, plane_val=Config.plane_val_, ax1_cut=Config.ax1_cut_, 
            ax2_cut=Config.ax2_cut_, multi_flag=Config.multi_flag_)
    else:
...
def multiread(runno=None, variables=None, first_slice=None, last_slice=None, 
    ...
    - multi_flag (bool): Whether to load multiple time slices.
    ...
    for filename in filenames: # Loop over variables
        ...
        with open(filename, 'rb') as bfile:
            ...
            for time_slice in time_range: # Loop over time slices
                ...
                else: # Appending for multiple time slices
                    time_slice_data.append(read_binary_f3d(bfile, time_slice, 
                        nx, ny, nz, xx, yy, zz, x_start, x_end, y_start, y_end, 
                        z_start, z_end, plane, plane_val, ax1_cut, ax2_cut, 
                        metadat_len_bytes))
            
                # Printing the filename and time read
                # timedt_ = time_slice * dtime
                # print(f'{filename} for the plane = {plane} and ' 
                # print(f'time = {timedt_:.5f} read!')
                # Printing the frame read
                print(f'frame = {time_slice} read!')
        variables_data.append(time_slice_data)
    ...
    grid_params = {'x':xx, 'y':yy, 'z':zz, 'nx': nx, 'ny': ny, 
            ...
            'time_slice': time_slice, 'tot_timeslices': ntimes, 
            'var_names': variables}
    
    # Data is populated to namespace if used as a script
    if __name__ == "__main__":
        globals().update({variable: data for variable, data in zip(variables, variables_data)})
        globals().update({key:value for key, value in grid_params.items()})
    else: # Data is returned as a dictionary if used as a function
        variables_data_dict = {variable: data for variable, data in zip(variables, variables_data)}
        # Combine dictionary with data dict and field dict
        variables_data_dict.update(grid_params)
        print(f"\nThe keys to access the fields in the data are: \n{variables_data_dict.keys()}")
        print(f"Each field variable has {ntimes} time slices.")
