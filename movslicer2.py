...
.npz or numpy compressed save file. This is quite useful if one wants to 
transfer huge binary data to local pc. 
PS: BEWARE THIS SCRIPT TAKES A LONG TIME IF .MOV FILE IS LARGE!
|Author: Regis | Date Created/Last Modified: Mar 17, 2025/Mar 18, 2025|
"""
#===============================================================================
#-------------------------------------------------------------------------------
# Imports
...
def multiread(runno=None, variables=None, first_slice=None, last_slice=None, 
    ...
    for indx, filename in enumerate(filenames): # Loop over variables
        ...
        out_folder = 'mov_npy' # Creating a folder to store .npy files
        os.makedirs(out_folder, exist_ok=True)
        with open(filename, 'rb') as bfile:
            npy_files = [] # list to store names of all npy files
            time_dict = {} # dictionary to store data of all time slices
            for time_slice in time_range: # Loop over time slices
                ...
                np.save(f'{out_folder}/{variables[indx]}{time_slice:04d}.npy', 
                        time_slice_data) # Saving each time slice as a .npy file
            
            # Sorting and memory-mapping .npy files
            npy_files = sorted(glob.glob(f'{out_folder}/*.npy'))
            print(f"Loading {len(npy_files)} files for compression...")
            time_dict['grid_params'] = grid_params
            for idx, npy_file in enumerate(npy_files):
                time_dict[f'{idx}'] = np.load(npy_file, mmap_mode='r')  # Memory-mapped

            # Save all memory-mapped arrays into a compressed .npz file
            print(f"Compressing data to {variables[indx]}.npz (this may take a while)...")
            np.savez_compressed(f'{variables[indx]}.npz', **time_dict)
            shutil.rmtree(out_folder) # Removing the .npy files folder
#-------------------------------------------------------------------------------
