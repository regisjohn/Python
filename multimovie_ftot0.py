...
#-------------------------------------------------------------------------------
def multimovie_ftot(runno=None, mov_vars=None, first_slice=None, 
              last_slice=None, skip_slice=None):
    ...
    def plot_slice_quiver(arr, arr1, arr2, title_lq, time_slice, v_min, v_max, output_dirq):
        ...
        quiv = axes.quiver(xx[::stride], zz[::stride], arr1T, arr2T, mag, cmap='binary',scale=75)
    ...
    # timedt_ = np.load(f'time_{run}.npy')
    ...
    for indx in plane_rng: # Looping over all planes
        ...
        for time_slice in time_range:
                print(time_slice,time_range)
                fig, axes = plt.subplots(1, 1) # Use a single figure object
                plot_slice_quiver(arr[time_slice], arr1[time_slice], 
                  arr2[time_slice], title_lq, time_slice, v_min_, v_max_, output_dir_q)
