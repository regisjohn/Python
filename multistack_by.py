...
#===============================================================================
"""
This is a python script for reading by.mov binary file from F3D program. Here 
we are trying to create a stack plot of by at x,z=0 in the x-z plane in a time 
vs y-location plot, meaning we are doing by vs y for all time slices.
The idea is for this to help us ascertain if there is magnetic pileup.
|Author: Regis | Date Created/Last Modified: Feb 20, 2025/Feb 23, 2025|
"""
#===============================================================================
#-------------------------------------------------------------------------------
# Imports
...
class Config:
    ...
    # Cuts and locations:
    plane_ = 'y' # Set to 'xy' or 'yz' or 'xz' or 'xyz'.
    ...
    # Plotting parameters:
    col_map = 'viridis' # Set to 'coolwarm' or 'viridis' or 'plasma' or 'inferno' or 'magma'
    savedpi = 300 # Set to the resolution of the saved plot.
#-------------------------------------------------------------------------------
# Main Function
#-------------------------------------------------------------------------------
def main():
...
def multiread(runno=None, variables=None, first_slice=None, last_slice=None, 
    ...
    if runno is None: 
        runno, first_slice, last_slice, skip_slice, filenames, nx,\
    ...
    by_vs_y_data = np.empty((len(time_range),ny))
    
    print('............')
    with open(filenames, 'rb') as bfile:
        time_slice_data = [] # list to store data of all time slices
        for time_slice in time_range: # Loop over time slices
            by_vs_y_data[time_slice] = read_binary_f3d(bfile, time_slice, 
    ...
    # Load time array:
    tt = np.load(f'time_{runno}.npy')
    
    # Save the data to a .npy file:
    np.save(f'by_yt_xz0_{runno}.npy', by_vs_y_data)
    ...
    def set_2dfig_size(fig, arr, base_sz = 6.4, cbar_space = 1.2):
        ...
        aspect_ratio = (arr.shape[1] / arr.shape[0])*1.2  # width/height
        # Set figure size
        fig.set_size_inches(base_sz * aspect_ratio, base_sz)
        
    # Create a stack plot of By in time vs y axis
    annot = input('\nEnter annotation: ')
    fig, ax = plt.subplots(1,1)
    set_2dfig_size(fig, by_vs_y_data) # Setting figure size
    img =ax.pcolormesh(yy, tt[:last_slice+1], by_vs_y_data, cmap=Config.col_map)
    ax.set_aspect('auto')
    cbar = fig.colorbar(img, ax=ax, format='%.1f')
    ax.set_xlabel('Distance Along Flux Rope')
    ax.set_ylabel('Time')
    ax.text(0.78, 0.88, f'{annot}', fontsize=12,
                        bbox=dict(facecolor='white', edgecolor='black'), 
                        transform=ax.transAxes)
    plt.tight_layout()
    plt.pause(0.00001) # Displaying plot interactively
    a = input('Enter 0 to continue: ')
    fig.savefig(f'by_yt_xz0_{runno}.png',dpi=Config.savedpi)
    plt.close()
...
def interactive_mode():
    ...
    filenames = f'by_{runno}.mov'
        
    # Read metadata from the first file
    with open(filenames, 'rb') as mfile:
        nx, ny, nz, dx, dy, dz, dtime, ntimes, metadat_len_bytes, xx, yy, \
            zz = read_first_metadata(mfile)
    # Displaying metadata
    print(f'\nInitial metadata read from : {filenames}!')
    print(f'nx={nx}, ny={ny}, nz={nz}, dx={dx}, dy={dy}, dz={dz}')
    print(f'\nTotal Times slices run from 0 to {ntimes-1}\n')
